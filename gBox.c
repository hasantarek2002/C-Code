...
int BoxWonOrNot(int pos, int type, char player)
{
	...
	{
		if((pos%14)!=13 && grid[pos-6]=='-' && grid[pos+8]=='-' && grid[pos+2]=='|' && grid[pos+1]==' ')  // right impo
		{
			cnt++;
            grid[pos+1]=player;
		}

		if((pos%14)!=7 && grid[pos-8]=='-' && grid[pos+6]=='-' && grid[pos-2]=='|' && grid[pos-1]==' ')  // left impo
		{
			cnt++;
            grid[pos-1]=player;
		}
...
void userInput()
{
	...
	while(1)
	{
		...
		printf("Enter firsr co-ordinates(i.e c1,c2):");
...
int main()
{
	...
	turn=get_random(number1,number2)%2;
	...
	while(1)
	{
		...
		if(turn==0)
		{
			...
			while(1)
			{
				...
				while(1)
				{
					y=(get_random(number1,number2)%50+get_random(number1,number2)%50)%50;

					for(i=0; i<16; i++)
					{
						if(valid[i]==y && x!=y) break;
					}
