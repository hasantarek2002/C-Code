...
bool valueOfNodeInBox(node* tNode,int ab){
   // printf(" [");
    int i ;
     //cout<<"xbfbf       "<<tNode->value.size()<<endl;
     for(i=0; i<tNode->value.size();i++){
        //printf("%d|",tNode->value[i]);
       // cout<<tNode->value[i]<<"   found"<<endl;
        if(tNode->value[i]==ab)
                 return true;
...
queue<pNode>q;
void bfsTraverse(node *tNode,int ab){
    ...
    while(!q.empty()){
        ...
        node *temp = p.tNode;
        q.pop();
        if(valueOfNodeInBox(temp,ab)){ cout<<"found"<<endl; return;}
